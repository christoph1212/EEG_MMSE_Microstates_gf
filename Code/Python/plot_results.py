...
check = (
    column_names_main[0:-3] == column_names_repli
).all()  # last three columns of column_names_main are post-hoc analyses done in main sample only
...
plt.xticks(
    np.arange(idx_vars.size), np.array(channel_names), fontsize="11", rotation=90
)
# plt.xlabel('Channel')
...
# Microstates
idx_order_ms = [4, 3, 1, 2, 0]  # microstates to order A,B,C,D,E
...
for axes_row in axes:
    ...
    mne.viz.plot_topomap(ms_maps[cnt], preproc.info, axes=axes_row, show=False)
...
# plt.grid(color='gray', linestyle='dashed', linewidth=0.5)
plt.savefig(
    "transitions_peak_RAPM_main.jpg", format="jpg", dpi=1200, bbox_inches="tight"
)
...
ax.spines["top"].set_visible(False)
plt.savefig(
    "transitions_peak_RAPM_repli.jpg", format="jpg", dpi=1200, bbox_inches="tight"
)
...
fig, (ax1) = plt.subplots(ncols=1)
im, cm = mne.viz.plot_topomap(
    np.array(df_main)[4, idx_vars], raw.info, axes=ax1, show=False, vmin=0.1, vmax=0.25
)
cbar = fig.colorbar(im)
cbar.ax.tick_params(labelsize=20)
ax1.spines["right"].set_visible(False)
...
fig, (ax1) = plt.subplots(ncols=1)
im, cm = mne.viz.plot_topomap(
    np.array(df_main)[3, idx_vars], raw.info, axes=ax1, show=False, vmin=0.1, vmax=0.9
)
cbar = fig.colorbar(im)
cbar.ax.tick_params(labelsize=20)
ax1.spines["right"].set_visible(False)
...
vmax = 0.9
im, cm = mne.viz.plot_topomap(
    var[:, scale[0]], raw.info, axes=ax1, show=False, vmin=vmin, vmax=vmax
)
im, cm = mne.viz.plot_topomap(
    var[:, scale[1]], raw.info, axes=ax2, show=False, vmin=vmin, vmax=vmax
)
im, cm = mne.viz.plot_topomap(
    var[:, scale[2]], raw.info, axes=ax3, show=False, vmin=vmin, vmax=vmax
)
im, cm = mne.viz.plot_topomap(
    var[:, scale[3]], raw.info, axes=ax4, show=False, vmin=vmin, vmax=vmax
)
im, cm = mne.viz.plot_topomap(
    var[:, scale[4]], raw.info, axes=ax5, show=False, vmin=vmin, vmax=vmax
)
