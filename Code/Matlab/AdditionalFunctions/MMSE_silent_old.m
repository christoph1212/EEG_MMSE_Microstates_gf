function MMSE_silent_old(IndexSubset, SubsetSize, dir_Raw, dir_Root, dir_Log, Parpoolsize, Overwrite)
...
parfor i_File = 1:length(IndexSubset)
    try
        ...
        if ~all(ismember(upper(AllChannels), upper({EEG.chanlocs.labels})))
            ...
            EEG = eeg_checkset( EEG );
            EEG = pop_interp(EEG, ToInterpolate, 'spherical');
