...
# data and crystallized intelligence. 
...
main_cond <- "first_run_eyes_closed"

if (!dir.exists(savepath)) {
  dir.create(savepath)
}

gc_score <- read_csv("Data/IST_table_crystallized.csv", 
                     col_select = c(1, 12))
...
for (data in data_types) {
  ...
  for (sample in subsamples) {
    ...
    if (data == "MMSE") {
      ...
      mmse_data_wide <- merge(mmse_data_wide, gc_score, by = "ID", all.x = TRUE)
      
      mmse_data_wide <- mmse_data_wide %>%
        filter(
          rowSums(!is.na(across(-c(ID, gc_score_z, Gender, Age)))) > 0
        )
      ...
      if (sample != "Full") {
        ...
        y <- mmse_data_wide_subsample$gc_score_z
      ...
      if (numberSignifComp == 0 && abs_min == 0) {
        ...
        tiff(paste(savepath, "PLSR_cryst_selected_comps_", main_cond, "_", data,
                   "_", sample, ".tiff", sep = ""), width = 2400, height = 1800,
             res = 600)
      ...
      microstate_data <- merge(microstate_data, gc_score, by = "ID", 
                               all.x = TRUE)
      ...
      if (sample != "Full") {
        ...
        y <- microstate_data_subsample$gc_score_z
...
max_slope <- c(0,0,0,0)
avg_ent <- c(0,0,0,0)
gc_score_z <- c(1,1,1,0)

mmse_path <- rbind(auc,max_slope, avg_ent, gc_score_z)
...
bar_mmse$Path <- sub("->gc_z", "", bar_mmse$Path)
...
ggsave(filename = paste0(savepath, "PLSPM_cryst_MMSE_" , main_cond, ".tiff"),
...
bar_ms$Path <- sub("->gc_z", "", bar_ms$Path)
...
ggsave(filename = paste0(savepath, "PLSPM_cryst_Microstates_" , main_cond,
